...
const generatePath = (path) => {
    ...
    let currenti = endingPointX;
    let currentj = endingPointY;

    while (!(currenti == startingPointX && currentj == startingPointY)) {
        if (path[currenti][currentj] == 'D') {
            currenti++;
        } else if (path[currenti][currentj] == 'U') {
            currenti--;
        } else if (path[currenti][currentj] == 'L') {
            currentj--;
        } else if (path[currenti][currentj] == 'R') {
            currentj++;
        }
        if (!(currenti == startingPointX && currentj == startingPointY)) {
            pathFromSrcToDest.push({ x: currenti, y: currentj });
        }
    }
...
const Dijkstra = async () => {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        distance[i] = new Array(cols).fill(INF);
    }
    let nodesToAnimate = [];
    ...
    const priorityQueue = new PriorityQueue(defaultNode, DijkstraComparator);

    priorityQueue.add({ distance: 0, x: startingPointX, y: startingPointY });
    distance[startingPointX][startingPointY] = 0;
    visited[startingPointX][startingPointY] = 1;
    ...
    while (!priorityQueue.isEmpty()) {
        ...
        for (let i = 0; i < 4; i++) {
            let newX = x + dx[i];
            let newY = y + dy[i];
            if (isValid(newX, newY) && isBlocked[newX][newY] === 0 && distance[x][y] + 1 < distance[newX][newY]) {
                priorityQueue.add({
                    distance: distance[x][y] + 1,
                    x: newX,
                    y: newY,
                });
                visited[newX][newY] = 1;
    ...
    if (visited[endingPointX][endingPointY]) {
        foundPath = true;
...
const AStar = async () => {
    ...
    const priorityQueue = new PriorityQueue(defaultNode, AStarComparator);
    priorityQueue.add({
    ...
    while (!priorityQueue.isEmpty()) {
        ...
        for (let i = 0; i < 4; i++) {
            let newX = x + dx[i];
            let newY = y + dy[i];
            if (isValid(newX, newY) && isBlocked[newX][newY] === 0 && visited[newX][newY] === 0) {
                let newNode = {};
                priorityQueue.add({
...
const algorithmDetails = {
    dijkstra: `
        <p class="font-bold text-lg text-teal-400">Dijkstra's Algorithm</p>
        <p><strong>Purpose:</strong> Finds the shortest path from a starting node to all other nodes in a graph.</p>
        <p class="mt-2"><strong>Key Features:</strong></p>
        <ul class="list-disc ml-4">
            <li>Uses a priority queue to explore nodes with the smallest cumulative distance first.</li>
            <li>Guarantees the shortest path for graphs with non-negative edge weights.</li>
        </ul>
        <p class="mt-2"><strong>Steps:</strong></p>
        <ol class="list-decimal ml-4">
            <li>Assign a tentative distance of 0 to the start node and ∞ to others.</li>
            <li>Process the closest unvisited node.</li>
            <li>Update distances to its neighbors if shorter paths are found.</li>
            <li>Repeat until all nodes are visited or the target is reached.</li>
        </ol>
        <p class="mt-2"><strong>Time Complexity:</strong> O((V+E)logV)</p>
        <p><strong>Space Complexity:</strong> O(V+E)</p>
    `,
    astar: `
        <p class="font-bold text-lg text-teal-400">A* Algorithm</p>
        <p><strong>Purpose:</strong> Finds the shortest path from a starting node to a target node efficiently.</p>
        <p class="mt-2"><strong>Key Features:</strong></p>
        <ul class="list-disc ml-4">
            <li>Uses a heuristic function to estimate the cost to the target.</li>
            <li>Combines Dijkstra's greedy search with heuristic-based guidance.</li>
        </ul>
        <p class="mt-2"><strong>Formula:</strong></p>
        <p>f(n) = g(n) + h(n)</p>
        <ul class="list-disc ml-4">
            <li><strong>f(n):</strong> Total estimated cost of a path through node n.</li>
            <li><strong>g(n):</strong> Cost to reach node n from the start.</li>
            <li><strong>h(n):</strong> Estimated cost from n to the target (heuristic).</li>
        </ul>
        <p class="mt-2"><strong>Steps:</strong></p>
        <ol class="list-decimal ml-4">
            <li>Initialize the open set with the start node.</li>
            <li>Process the node with the lowest f(n).</li>
            <li>Update neighbors with new g(n) and f(n) if shorter paths are found.</li>
            <li>Repeat until the target is reached or the open set is empty.</li>
        </ol>
        <p class="mt-2"><strong>Time Complexity:</strong> O((V+E)logV)</p>
...
const algorithmDropdown = document.getElementById("algorithm_type");
const contentElement = document.getElementById("algorithm-content");

algorithmDropdown.addEventListener("change", () => {
    const selectedAlgorithm = algorithmDropdown.value;
    contentElement.innerHTML = algorithmDetails[selectedAlgorithm];
});